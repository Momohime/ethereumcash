...
int nAnonymizeethereumcashAmount = 1000;
...
#else
    const char* pszModule = "ethereumcash";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\EthereumCash
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\EthereumCash
    // Mac: ~/Library/Application Support/EthereumCash
    // Unix: ~/.ethereumcash
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "EthereumCash";
#else
    fs::path pathRet;
...
void createConf()
{
	...
	const char* nodes =  "\nrpcport=23233"
					     ...
					     "\naddnode=node1.ethereumcash.io"
						 "\naddnode=node2.ethereumcash.io"
						 "\naddnode=node3.ethereumcash.io"
						 "\naddnode=node4.ethereumcash.io"
						 "\naddnode=node5.ethereumcash.io";
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "ethereumcash.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "ethereumcashd.pid"));
