...
void Shutdown()
{
    ...
    RenameThread("ethereumcash-shutoff");
...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: ethereumcash.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: ethereumcashd.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizeethereumcashamount=<n> " + _("Keep N EthereumCash anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. EthereumCash is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. EthereumCash is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("EthereumCash version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "EthereumCash server starting\n"); 
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of EthereumCash") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart EthereumCash to complete") << "\n";
    ...
    nAnonymizeethereumcashAmount = GetArg("-anonymizeethereumcashamount", 0);
    if(nAnonymizeethereumcashAmount > 999999) nAnonymizeethereumcashAmount = 999999;
    if(nAnonymizeethereumcashAmount < 2) nAnonymizeethereumcashAmount = 2;
    ...
    LogPrintf("Anonymize EthereumCash Amount %d\n", nAnonymizeethereumcashAmount);

    /* Denominations
       A note about convertability. Within Darksend pools, each denomination
       is convertable to another.
       For example:
       1ETHC+1000 == (.1ETHC+100)*10
       10ETHC+10000 == (1ETHC+1000)*10
    */
