...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("ethereumcash"))
        return false;
...
{
    // Convert ethereumcash:// to ethereumcash:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("ethereumcash://", Qt::CaseInsensitive))
    {
        uri.replace(0, 11, "ethereumcash:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "EthereumCash.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for EthereumCash.lnk
...
{
    return GetAutostartDir() / "ethereumcash.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a ethereumcash.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=EthereumCash\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the ethereumcash app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add ethereumcash app to startup item list
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("EthereumCash-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  ethereumcash-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("EthereumCash-Qt"));
