...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeethereumcashAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeethereumcashAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeethereumcashAmount = strAnonymizeethereumcashAmount.remove(strAnonymizeethereumcashAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeethereumcashAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeethereumcashAmount*COIN) nMaxToAnonymize = nAnonymizeethereumcashAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeethereumcashAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeethereumcashAmount));
        strAnonymizeethereumcashAmount = strAnonymizeethereumcashAmount.remove(strAnonymizeethereumcashAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeethereumcashAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
...
void OverviewPage::toggleDarksend(){
    ...
    if(!fEnableDarksend){
        ...
        if(nAnonymizeethereumcashAmount == 0){
