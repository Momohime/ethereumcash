...
//
// Bootup the masternode, look for a 25000 ETHC input and register on the network
//
void CActiveMasternode::ManageStatus()
{
    ...
    if(status == MASTERNODE_NOT_PROCESSED) {
        ...
        if(GetMasterNodeVin(vin, pubKeyCollateralAddress, keyCollateralAddress)) {
            ...
            /* rewards are not supported in ethereumcash.conf */
...
{
    CethereumcashcoinAddress address(collateralAddress);
